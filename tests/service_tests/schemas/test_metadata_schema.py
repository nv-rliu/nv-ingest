...
def test_chart_metadata_schema_defaults():
    config = ChartMetadataSchema(table_format=TableFormatEnum.HTML)
    assert config.caption == ""
    assert config.table_content == ""
...
def test_error_metadata_schema_defaults():
    config = ErrorMetadataSchema(task=TaskTypeEnum.EMBED, status=StatusEnum.ERROR, error_msg="An error occurred.")
...
def test_info_message_metadata_schema_defaults():
    config = InfoMessageMetadataSchema(
        task=TaskTypeEnum.SPLIT, status=StatusEnum.SUCCESS, message="This is an info message.", filter=False
    )
