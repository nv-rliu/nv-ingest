...
@patch(f"{MODULE_UNDER_TEST}.charset_normalizer.detect")
...
@patch(f"{MODULE_UNDER_TEST}.magic.from_file", return_value="unknown")  # Mock magic.from_file to return 'text/plain'
...
@patch(f"{MODULE_UNDER_TEST}.magic.from_file", return_value="text/plain")  # Mock magic.from_file to return 'text/plain'
...
@pytest.mark.skip("Disabled while we figure out why libmagic is missing on the CI system")
@patch(f"{MODULE_UNDER_TEST}.magic.from_file")
